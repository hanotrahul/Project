...
# use_session_with_seed(seed, disable_gpu = TRUE,
#   disable_parallel_cpu = TRUE, quiet = FALSE)
# reset_default_graph()
...
class mask_rcnn_grading(object):
    ...
    def detection(self,contour_path,model):
        # thread_graph = Graph()
    ...
    # def detection2(self,semi_contour,model,xx):
    #     for i in semi_contour:
    #         with graph.as_default():
    ...
    #             result['class_ids'] = np.array(result['class_ids'])
    #             result['masks'] = np.array(result['masks'])
    #             detection_results[i]=result 
    #             print(xx,"<-batch is "," time is :: ",( e_time-s_t ),i,">",detection_results[i])
    ...
    def get_layer2_mask_object_detection_results_(self, contour_details, final_result_dict, ratio, model, contour_images_folder, original_image_path, image_contours):
        ...
        while i<len(contour_paths):
            ...
            st=time.time()
            cidex=[]
            while idex<4 and i<len(contour_paths):
             cidex.append(contour_paths[i])
             idex=idex+1
             i=i+1
            clist.append(cidex)
            
            #  ti[idex]=threading.Thread(target=self.detection,args=(contour_paths[i],model))
            #  ti[idex].start()
             #ti[idex].run()
            ...
            # didex=0
            # print("no of active threading >>>",threading.enumerate() ,">>>>>>>>>>>>>>>>>>>>>>>")
            # while (didex<4)and cidex<len(contour_paths):
            #     ti[didex].join()   
            #     didex=didex+1
            #     print("cidex",cidex)
            #     cidex += 1
            # timing[rit]=time.time()-st
            # rit=rit+1
        ...
        for i in clist:
            print(i)
            th[xx]=threading.Thread(target=self.detection,args=(i,xx))
            th[xx].start()
            xx=xx+1
...
if __name__ == '__main__':
    ...
    class ClassificationConfig(Config):
        GPU_COUNT = 1
        IMAGES_PER_GPU = 1
        # BATCH_SIZE=len(image_contours.keys())
        NAME = configfile['category']
